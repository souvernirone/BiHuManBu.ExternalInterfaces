    ...
    public class AgentRepository : IAgentRepository
    {
        ...
        public List<string> GetSonsList(int currentAgent, bool isContainSelf)
        {
            ...
            try
            {
                ...
                #region SQL语句
                var strSql = GetAgentSonsSql(isContainSelf);
                #endregion
                //查询列表
        ...
        public List<int> GetSonsIntList(int currentAgent, bool isContainSelf)
        {
        ...
        /// 获取子集的sql
        /// </summary>
        /// <param name="isContainSelf"></param>
        /// <returns></returns>
        private StringBuilder GetAgentSonsSql(bool isContainSelf)
        {
        ...
        public List<bx_agent> GetAgentList(int curAgent, int topAgent, string search, int agentStatus, int curPage, int pageSize, out int totalCount)
        {
            ...
            try
            {
                ...
                List<int> reSonId = GetSonsIntList(curAgent, false);
                var list1 = DataContextFactory.GetDataContext().bx_agent.Where(p => reSonId.Contains(p.Id)
                                                        && (string.IsNullOrEmpty(search) || p.AgentName.Contains(search))
                                                        && (agentStatus == -1 || p.IsUsed == agentStatus)
                    ).ToList();
