...
namespace BiHuManBu.ExternalInterfaces.Repository
{
    public class UserinfoExpandRepository : IUserinfoExpandRepository
    {
        ...
        public bool UpdateCameraTimeByBuid(string buids, DateTime UpdateTime, string CameraId)
        {
            try
            {
                var param = new[]
                ...
                string sql = "UPDATE bx_userinfo_expand SET CameraId=@camera_id,update_time=@update_time WHERE b_uid IN (" + buids + ");";
                return _context.Database.ExecuteSqlCommand(sql, param) > 0;
            }
            catch (Exception ex)
            {
                logError.Error("UpdateCameraTimeByBuid发生异常:" + ex.Source + "\n" + ex.StackTrace + "\n" + ex.Message + "\n" + ex.InnerException);
            }
            return false;
        }
        /// <summary>
        ...
        public bool UpdateUserExpandByBuid(string buids, int DeleteType, DateTime DeleteTime)
        {
            ...
            {
                logError.Error("UpdateUserExpandByBuid发生异常:" + ex.Source + "\n" + ex.StackTrace + "\n" + ex.Message + "\n" + ex.InnerException);
            }
            return false;
        }
        ...
        public bool UpdateUserExpandByBuid(List<long> buids, int DeleteType, DateTime DeleteTime)
        {
            ...
            {
                logError.Error("UpdateUserExpandByBuid:" + string.Join(",", buids) + "发生异常:" + ex.Source + "\n" + ex.StackTrace + "\n" + ex.Message + "\n" + ex.InnerException);
            }
            return false;
        }
        /// <summary>
        ...
        public List<long> GetExistBuidList(string buids)
        {
            ...
            {
                logError.Error("GetExistBuidList发生异常:" + ex.Source + "\n" + ex.StackTrace + "\n" + ex.Message + "\n" + ex.InnerException);
            }
            return null;
        }
                ...
                lock (syncObject)
                {
                    if (Get(buid) == null)
                    {
                        try
                        {
                            ...
                            bool result = false;
                            string sql = "INSERT INTO bx_userinfo_expand(b_uid,update_time) values(@b_uid,@update_time) ;";
                            result = _context.Database.ExecuteSqlCommand(sql, param) > 0;
                            return result;
                        }
                        catch (Exception ex)
                        {
                            logError.Error("添加userinfo_expand发生异常:" + ex.Source + "\n" + ex.StackTrace + "\n" + ex.Message + "\n" + ex.InnerException);
                        }
                    }
                }
            }
